...
ANDROID_NDK_VERSION_64=${2:-23.2.8568313}
ANDROID_NDK_VERSION_32=${3:-23.2.8568313}
...
for ABI in x86 armeabi-v7a x86_64 arm64-v8a ; do
  case ${ABI} in
    arm64-v8a | x86_64)
      ANDROID_NDK_VERSION=$ANDROID_NDK_VERSION_64
      ANDROID_PLATFORM="android-21"
    ;;
    armeabi-v7a | x86)
      ANDROID_NDK_VERSION=$ANDROID_NDK_VERSION_32
  ...
  echo "Building TDLib for ${ABI} with NDK ${ANDROID_NDK_VERSION}..."
  
  mkdir -p build-$ABI || exit 1
  cd build-$ABI
  cmake -DCMAKE_SHARED_LINKER_FLAGS="-Wl,--gc-sections,--icf=safe" -DCMAKE_TOOLCHAIN_FILE="$ANDROID_NDK_ROOT/build/cmake/android.toolchain.cmake" -DOPENSSL_ROOT_DIR="$OPENSSL_INSTALL_DIR/$ABI" -DCMAKE_BUILD_TYPE=RelWithDebInfo -GNinja -DANDROID_ABI=$ABI -DANDROID_STL=$ANDROID_STL -DANDROID_PLATFORM=$ANDROID_PLATFORM .. || exit 1
  cmake --build . || exit 1
  cd ..
